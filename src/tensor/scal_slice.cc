 ...
 *
 * @file src/tensor/scal_slice.cc
 * Tensor wrappers for addition of a tensor and a broadcasted slice
 *
 * @version 1.1.0
 * */

#include "nntile/tensor/scal_slice.hh"
...
template<typename T>
void scal_slice_async(Scalar alpha, const Tensor<T> &src, const Tensor<T> &dst,
        Index axis)
 ...
 * */
{
    ...
    // Apply per-tile scal_slice asynchronously as needed
    ...
    for(Index i = 0; i < src.grid.nelems; ++i)
    {
        ...
        for(Index j = 0; j < dst.grid.shape[axis]; ++j)
        {
            ...
            if(mpi_rank == dst_tile_rank)
            {
                ...
                starpu::scal_slice::submit<T>(m, n, k, alpha, src_tile_handle,
                        dst_tile_handle);
            }
...
template<typename T>
void scal_slice(Scalar alpha, const Tensor<T> &src, const Tensor<T> &dst, Index axis)
//! Tensor<T> addition of a tensor and a broadcasted slice
/*! Blocking version of scal_slice_async<T>.
 * Reshapes input tensor and slice into 3-dimensional and 2-dimensional arrays
 * and performs the following operations:
 *      dst[i,l,j,b] = alpha * src[i,j,b]
 *
 ...
 *  */
{
    scal_slice_async<T>(alpha, src, dst, axis);
...
template
void scal_slice_async<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src,
        const Tensor<fp32_t> &dst, Index axis);

template
void scal_slice_async<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src,
        const Tensor<fp32_fast_tf32_t> &dst, Index axis);

template
void scal_slice_async<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src,
        const Tensor<fp64_t> &dst, Index axis);

template
void scal_slice_async<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src,
        const Tensor<bf16_t> &dst, Index axis);

template
void scal_slice_async<fp32_fast_fp16_t>(Scalar alpha, const Tensor<fp32_fast_fp16_t> &src,
        const Tensor<fp32_fast_fp16_t> &dst, Index axis);

template
void scal_slice_async<fp32_fast_bf16_t>(Scalar alpha, const Tensor<fp32_fast_bf16_t> &src,
        const Tensor<fp32_fast_bf16_t> &dst, Index axis);

// Explicit instantiation of template
template
void scal_slice<fp32_t>(Scalar alpha, const Tensor<fp32_t> &src,
        const Tensor<fp32_t> &dst, Index axis);

template
void scal_slice<fp32_fast_tf32_t>(Scalar alpha, const Tensor<fp32_fast_tf32_t> &src,
        const Tensor<fp32_fast_tf32_t> &dst, Index axis);

template
void scal_slice<fp64_t>(Scalar alpha, const Tensor<fp64_t> &src,
        const Tensor<fp64_t> &dst, Index axis);

template
void scal_slice<fp32_fast_fp16_t>(Scalar alpha, const Tensor<fp32_fast_fp16_t> &src,
        const Tensor<fp32_fast_fp16_t> &dst, Index axis);

template
void scal_slice<fp32_fast_bf16_t>(Scalar alpha, const Tensor<fp32_fast_bf16_t> &src,
        const Tensor<fp32_fast_bf16_t> &dst, Index axis);

template
void scal_slice<bf16_t>(Scalar alpha, const Tensor<bf16_t> &src,
        const Tensor<bf16_t> &dst, Index axis);
