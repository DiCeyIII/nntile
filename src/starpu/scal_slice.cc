...
//! StarPU wrappers for scal_slice operation
namespace nntile::starpu::scal_slice
{

//! StarPU wrapper for kernel::scal_slice::cpu<T>
template<typename T>
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::scal_slice::cpu<T>(args->m, args->n, args->k, args->alpha, src,
            dst);
#endif // STARPU_SIMGRID
}

#ifdef NNTILE_USE_CUDA
//! StarPU wrapper for kernel::scal_slice::cuda<T>
template<typename T>
    ...
    noexcept
{
    ...
    // Launch kernel
    kernel::scal_slice::cuda<T>(stream, args->m, args->n, args->k, args->alpha,
            src, dst);
#endif // STARPU_SIMGRID
}
#endif // NNTILE_USE_CUDA

//! Footprint for scal_slice tasks
static
...
void init()
{
    codelet_fp32.init("nntile_scal_slice_fp32",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_bf16.init("nntile_scal_slice_bf16",
            footprint,
            {cpu<bf16_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<bf16_t>}
    ...
    codelet_fp32_fast_tf32.init("nntile_scal_slice_fp32_fast_tf32",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp64.init("nntile_scal_slice_fp64",
            footprint,
            {cpu<fp64_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp64_t>}
    ...
    codelet_fp32_fast_fp16.init("nntile_scal_slice_fp32_fast_fp16",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
    ...
    codelet_fp32_fast_bf16.init("nntile_scal_slice_fp32_fast_bf16",
            footprint,
            {cpu<fp32_t>},
#ifdef NNTILE_USE_CUDA
            {cuda<fp32_t>}
...
//! Insert scal_slice task into StarPU pool of tasks
 ...
 * */
{
    ...
    if(ret != 0)
    {
        throw std::runtime_error("Error in scal_slice task submission");
    }
}
