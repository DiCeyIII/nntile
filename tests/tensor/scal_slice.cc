 ...
 *
 * @file tests/tensor/scal_slice.cc
        ...
        Index axis)
{
    ...
    // Perform tensor-wise and tile-wise scal_slice operations
    scal_slice<T>(-1.0, src, dst, axis);
    if(mpi_rank == mpi_root)
    {
        tile::scal_slice<T>(-1.0, src_single.get_tile(0),
...
void validate()
{
    ...
    TEST_THROW(scal_slice<T>(1.0, A, C, 0));
    TEST_THROW(scal_slice<T>(1.0, A, B, -1));
    TEST_THROW(scal_slice<T>(1.0, B, A, 2));
    TEST_THROW(scal_slice<T>(1.0, B, C, 0));
    TEST_THROW(scal_slice<T>(1.0, C, A, 1));
    TEST_THROW(scal_slice<T>(1.0, C, B, 0));
    TEST_THROW(scal_slice<T>(1.0, A, B, 1));
}
...
int main(int argc, char **argv)
{
    ...
    starpu::scal::init();
    starpu::add::init();
    starpu::scal_slice::init();
    starpu::subcopy::init();
    starpu::copy::init();
    starpu::scal_slice::restrict_where(STARPU_CPU);
